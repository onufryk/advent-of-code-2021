...
class Solution:
    ...
    def read_input(self):
        ...
        for input_line in input_file:
            ...
            if start[0] == end[0]:
                if start[1] > end[1]:
                    self.lines.append((end, start, 0))
                else:
                    self.lines.append((start, end, 0))
            elif start[1] == end[1]:
                if start[0] > end[0]:
                    self.lines.append((end, start, 1))
                else:
                    self.lines.append((start, end, 1))
    ...
    def calculate(self):
        ...
        for i in range(len(self.lines)):
            ...
            print("Line {} - {} ({}).".format(start_i, end_i, 'H' if dir_i else 'V'))
            if dir_i == 1:
